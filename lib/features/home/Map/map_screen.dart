...
class _MapScreenState extends ConsumerState<MapScreen> {
  ...
  String _getActionButtonText(OrderModel order) {
    if (order.isMultiStop) {
      ...
      final stopType = activeStop.stopType.replaceAll("_", " ").toUpperCase();
      final stopNo = activeStop.sequenceNumber;

      // 🔹 Stop already arrived → complete
      if (activeStop.status == "arrived" ||
          activeStop.status == "in_progress") {
        return "Complete $stopType";
      }

      // 🔹 Not arrived yet
      return "Start $stopType $stopNo";
    }
