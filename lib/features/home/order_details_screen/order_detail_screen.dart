                ...
                if (order.isMultiStop)
                  Builder(
                    builder: (context) {
                      ...
                      /// ❌ agar koi stop skipable nahi hai to button hide
                      if (availableStops.isEmpty) {
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                /// 🔵 STOP TYPE BADGE (Pickup / Waypoint / Drop-off)
                stopType != null
                    ? stopTypeBadge(stopType)
                    : Text(
                        title,
                        style: TextStyle(
                          fontSize: 16,
  ...
  Widget _actionButtons(BuildContext context, WidgetRef ref, OrderModel order) {
    ...
    if (order.isMultiStop &&
      ...
      if (isDriverOffDuty()) {
        ...
        final stopType = activeStop?.stopType.toUpperCase();
        return mainButton("Start $stopType", () async {
...
Widget stopTypeBadge(String type) {
  Color color;
  IconData icon;
